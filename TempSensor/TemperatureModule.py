...
def temperature_monitor(is_command):
    ...
    while tries < 3:
        try:
            ...
            # CONSOLE PRINT
            print(str(temperature) + " Fahrenheit")
            print(str(rel_humidity) + "% Relative Humidity")

            # HEAT INDEX FORMULA
            heat_index = -42.379 + 2.04901523 * temperature + 10.14333127 * rel_humidity - 0.22475541 * temperature * rel_humidity - 0.00683783 * temperature * temperature - 0.05481717 * rel_humidity * rel_humidity + 0.00122874 * temperature * temperature * rel_humidity + 0.00085282 * temperature * rel_humidity * rel_humidity - 0.00000199 * temperature * temperature * rel_humidity * rel_humidity
            print(ceil(heat_index))

            # ADJUSTMENT
            if rel_humidity < 13 and 80 < temperature < 112:
                heat_index -= [(13 - rel_humidity) / 4] * sqrt([17 - abs(temperature - 95)] / 17)
            elif rel_humidity > 85 and 80 < temperature < 87:
                heat_index += [(rel_humidity - 85) / 10] * [(87 - temperature) / 5]
            ...
            if heat_index < 89:
                # Normal temperature
                print(str(heat_index) + "C, Normal")
                if is_command:
                    push_message("Current Temperature", str(heat_index) + "C, Normal")
            elif heat_index < 103:
                # Uncomfortable temperature
                print(str(heat_index) + "C, Uncomfortable")
                if is_command:
                    push_message("Current Temperature", str(heat_index) + "C, Uncomfortable")
            elif heat_index < 124:
                # Dangerous temperature
                print(str(heat_index) + "C, Dangerous")
                if is_command:
                    push_message("Current Temperature", str(heat_index) + "C, Dangerous")
            else:
                # Probably dead
                print(str(heat_index) + "C, Might not want to come home")
                if is_command:
                    push_message("Current Temperature", str(heat_index) + "C, Might not want to come home")
